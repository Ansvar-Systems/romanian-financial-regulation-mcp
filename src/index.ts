...
/**
 * Romanian Financial Regulation MCP — stdio entry point.
 *
 * Provides MCP tools for querying ASF (Autoritatea de Supraveghere Financiara)
 * and BNR (Banca Nationala a Romaniei) regulations: provisions, sourcebooks,
...
const SERVER_NAME = "romanian-financial-regulation-mcp";

// ─── Tool definitions ────────────────────────────────────────────────────────

const TOOLS = [
  {
    name: "ro_fin_search_regulations",
    description:
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: "string",
          description: "Search query in Romanian (e.g., 'cerinte de capital', 'piata de capital', 'cerinte prudentiale')",
        },
        sourcebook: {
          type: "string",
          description: "Filter by sourcebook ID (e.g., ASF_NORME, ASF_INSTRUCTIUNI, BNR_REGULAMENTE, BNR_NORME). Optional.",
        },
        status: {
          type: "string",
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        sourcebook: {
          type: "string",
          description: "Sourcebook identifier (e.g., ASF_NORME, BNR_REGULAMENTE)",
        },
        reference: {
          type: "string",
          description: "Full provision reference (e.g., 'ASF_NORME 15/2015 art.3')",
        },
      },
  ...
  {
    name: "ro_fin_search_enforcement",
    description:
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: "string",
          description: "Search query (e.g., firm name, type of breach, 'abuz de piata')",
        },
        action_type: {
          type: "string",
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case "ro_fin_search_regulations": {
      ...
      case "ro_fin_about": {
        return textContent({
          ...
          version: pkgVersion,
          description:
            "Romanian Financial Regulation MCP server. Provides access to ASF (Autoritatea de Supraveghere Financiara) norme on capital markets and BNR (Banca Nationala a Romaniei) regulamente on prudential requirements.",
          data_sources: [
            "ASF (https://www.asfromania.ro/)",
